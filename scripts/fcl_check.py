#!/usr/bin/env python3
"""
FCL-based Collision Checker for Robot Trajectories

This script checks for collisions along a robot trajectory using python-fcl.
...
Usage:
    python fcl_check.py --trajectory data/trajectory/joint_trajectory_dp_5000_base.csv \
                        --robot_urdf data/input/ur20.urdf \
                        --mesh data/input/glass_o3d.obj
"""
...
class FCLCollisionChecker:
    ...
    def __init__(self, robot_urdf_path: str, obstacle_mesh_paths: List[str],
        ...
        for mesh_path in obstacle_mesh_paths:
            ...
            if mesh is not None:
                ...
                # Create FCL transform for glass position
                glass_transform = fcl.Transform(
                    np.eye(3),  # No rotation
                    glass_position  # Translation
                )

                # Create FCL collision object with transform
                col_obj = self._create_fcl_collision_object(mesh, glass_transform)
    ...
    def _create_fcl_collision_object(
        self,
        mesh: trimesh.Trimesh,
        transform: Optional[fcl.Transform] = None
    ) -> fcl.CollisionObject:
        """Create FCL collision object from trimesh"""
        ...
        return fcl.CollisionObject(bvh, transform)
    ...
    def _create_robot_collision_geometry(
        ...
        Returns:
            List of FCL collision objects representing robot links
        """
    ...
    def _create_mesh_collision_objects(self) -> List[fcl.CollisionObject]:
        """Create FCL collision objects using actual link meshes"""
        ...
        for link_name, mesh in self.link_meshes.items():
            ...
            # Create FCL mesh collision object
            vertices = np.array(mesh.vertices, dtype=np.float64)
            triangles = np.array(mesh.faces, dtype=np.int32)
            ...
            # Create transform
            fcl_transform = fcl.Transform(rotation, position)

            # Create collision object
            col_obj = fcl.CollisionObject(bvh, fcl_transform)
            robot_collision_objects.append(col_obj)

        return robot_collision_objects

    def _create_sphere_collision_objects(self) -> List[fcl.CollisionObject]:
        """Create FCL collision objects using spheres from YAML config"""
        robot_collision_objects = []
        ...
        for link_name, spheres in self.collision_spheres.items():
            ...
            for sphere_def in spheres:
                ...
                # Create FCL sphere
                fcl_transform = fcl.Transform(center_world)
                sphere = fcl.Sphere(radius)
                col_obj = fcl.CollisionObject(sphere, fcl_transform)
                robot_collision_objects.append(col_obj)

        return robot_collision_objects

    def _create_capsule_collision_objects(self) -> List[fcl.CollisionObject]:
        """Create FCL collision objects using capsule approximations"""
        robot_collision_objects = []
        ...
        for capsule_def in self.link_capsules:
            ...
            capsule = fcl.Capsule(self.capsule_radius, length)
            col_obj = fcl.CollisionObject(capsule, fcl_transform)
            robot_collision_objects.append(col_obj)
...
def generate_interpolated_path(
    ...
    Args:
        start: Starting joint configuration (6 values)
        end: Ending joint configuration (6 values)
        num_steps: Number of intermediate steps to generate

    Returns:
        List of interpolated joint configurations (does not include start/end)
    """
    ...
    # Generate interpolation parameters (excluding 0.0 and 1.0)
    alphas = np.linspace(0.0, 1.0, num_steps + 2, endpoint=True)[1:-1]

    # Linear interpolation: q(t) = start + t * (end - start)
    interpolated = [start + alpha * (end - start) for alpha in alphas]
...
def main():
    parser = argparse.ArgumentParser(description='Check trajectory collisions using FCL')
    parser.add_argument(
        '--trajectory',
        type=str,
    ...
    parser.add_argument(
        '--robot_urdf',
        type=str,
        default='ur_description/ur20.urdf',
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['data/input/object/glass_zup.obj'],  # Z-up for Pinocchio/FCL (same as Isaac Sim)
        help='Paths to obstacle mesh files (use Z-up coordinates to match URDF)'
    )
    parser.add_argument(
        '--robot_config',
        type=str,
        default='ur_description/ur20.yml',
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help='Number of interpolation steps between waypoints (default: 60, matching run_app_v3.py)'
    )

    args = parser.parse_args()

    print("=" * 70)
    print("FCL Collision Checker for Robot Trajectories")
    print("=" * 70)
    ...
    print("  2. Adjust --interp-steps to control interpolation density (default: 60)")
