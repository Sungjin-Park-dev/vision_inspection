...
Loads pre-computed viewpoints (from mesh_to_viewpoints.py) or samples points from mesh,
then solves the Traveling Salesman Problem to find an optimal visit order.

Supported algorithms:
- Nearest Neighbor (NN): Fast greedy heuristic
- Random Insertion (RI): Better quality, slower
- 2-opt refinement: Optional post-processing

Coordinate system:
- Input: Z-up coordinates (from mesh_to_viewpoints.py or direct mesh)
...
# read_pcd_file_simple() removed - Open3D is now used for all point cloud operations
...
def main():
    ...
    parser.add_argument('--mesh_file', type=str,
                        ...
                        help='Path to mesh file (.obj or .pcd, Z-up coordinate system). If not provided, uses random points.')
