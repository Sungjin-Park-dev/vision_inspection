...
class WorldState:
    ...
    robot_state: dict  # Store robot config for later use
...
def setup_motion_gen(
    ...
    """Setup MotionGen for trajectory planning

    Returns:
        MotionGen instance configured with robot and world
    """
    print_section_header("SETTING UP MOTION GENERATOR", width=60)
    ...
    world_cfg = setup_collision_world(
        ...
        robot_mount_position=cfg.robot_mount_position,
        robot_mount_dimensions=cfg.robot_mount_dimensions,
        mesh_files=[config.DEFAULT_MESH_FILE],  # Include glass mesh
        verbose=False
    )
    ...
    print_key_value("MotionGen initialized", "Success")
    print_key_value("Collision checker type", "MESH")
    print_key_value("CUDA graph enabled", "True")
    print()

    # Update obstacles from USD stage
    usd_helper = UsdHelper()
    usd_helper.load_stage(my_world.stage)
    ...
    print_key_value("World obstacles updated", "Success")
    print()

    return motion_gen


def plan_initial_trajectory(
    motion_gen: MotionGen,
    start_config: np.ndarray,
    goal_config: np.ndarray
) -> List[np.ndarray]:
    """Plan trajectory from start to goal using MotionGen
    ...
    Returns:
        List of joint configurations along the planned trajectory
    """
    print_section_header("PLANNING INITIAL TRAJECTORY", width=60)
    print_key_value("Start config", start_config)
    print_key_value("Goal config", goal_config)
    print()
    ...
    start_state = JointState.from_position(
        tensor_args.to_device(start_config.reshape(1, -1))
    )

    goal_state = JointState.from_position(
        tensor_args.to_device(goal_config.reshape(1, -1))
    )
    ...
    if result.success.item():
        ...
        joint_trajectory = trajectory.position.cpu().numpy()

        print_key_value("Planning status", "Success")
        print_key_value("Trajectory waypoints", len(joint_trajectory))
        print_key_value("Trajectory duration", f"{len(joint_trajectory) * 0.02:.2f}s")
        print()

        # Convert to list of numpy arrays
        return [joint_trajectory[i] for i in range(len(joint_trajectory))]
    else:
        print_key_value("Planning status", "Failed")
        print("Warning: Could not plan trajectory to first waypoint")
        print("Robot will jump directly to first waypoint")
        print()
        return []
...
def initialize_simulation(cfg: SimulationConfig) -> WorldState:
    ...
    # Setup MotionGen for planning initial trajectory
    motion_gen = setup_motion_gen(my_world, robot_state, cfg)
...
def main():
    ...
    # Plan initial trajectory from robot's default position to first waypoint
    if len(joint_targets) > 0:
        # Get robot's default/retract configuration
        robot_cfg = world_state.robot_state['robot_cfg']
        start_config = np.array(robot_cfg["kinematics"]["cspace"]["retract_config"], dtype=np.float64)
        # start_config = np.array([-2.0, -2.0, -1.0 ,-1.5 , 0.3, 0.0])
        goal_config = joint_targets[0]

        # Plan trajectory using MotionGen
        initial_trajectory = plan_initial_trajectory(
            world_state.motion_gen,
            start_config,
            goal_config
        )

        # Prepend initial trajectory to joint targets
        if len(initial_trajectory) > 0:
            print_section_header("COMBINING TRAJECTORIES", width=60)
            print_key_value("Initial trajectory waypoints", len(initial_trajectory))
            print_key_value("CSV trajectory waypoints", len(joint_targets))

            # Combine trajectories (skip last point of initial trajectory to avoid duplicate)
            joint_targets = initial_trajectory[:-1] + joint_targets

            print_key_value("Total trajectory waypoints", len(joint_targets))
            print()
        else:
            print_section_header("TRAJECTORY INFO", width=60)
            print("Using CSV trajectory only (initial planning failed)")
            print_key_value("Total waypoints", len(joint_targets))
            print()
