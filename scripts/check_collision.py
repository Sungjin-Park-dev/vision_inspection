...
# ============================================================================
def main():
    """Main entry point - performs same tasks as notebook Section 4"""
    parser = argparse.ArgumentParser(
        description="Check robot trajectory for collisions using CuRobo"
    )
    parser.add_argument(
        "--object_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to trajectory CSV file (e.g., data/glass/trajectory/500/gtsp.csv). "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to obstacle mesh file (default: use object mesh or config.DEFAULT_MESH_FILE)"
    )
    parser.add_argument(
        "--exclude_last_joint",
        action="store_true",
        help="Exclude last joint from interpolation checks (default: from config)"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if args.object_name is None and args.trajectory is None:
        parser.error("Either --object_name (with --num_viewpoints) or --trajectory must be provided")
    ...
    if args.object_name:
        if args.trajectory is None:
            args.trajectory = str(config.get_trajectory_path(args.object_name, args.num_viewpoints, "gtsp.csv"))
            print(f"Using auto-generated trajectory path: {args.trajectory}")

    # Use default mesh if not specified
    if args.mesh is None:
        if args.object_name:
            # Use object-specific mesh
            args.mesh = str(config.get_mesh_path(args.object_name))
        else:
            # Use default mesh from config
            args.mesh = config.DEFAULT_MESH_FILE
    mesh_file = args.mesh

    # Auto-generate collision-free trajectory path
    traj_dir = os.path.dirname(args.trajectory)
    if args.object_name:
        # New structure: save as gtsp_final.csv
        collision_free_csv = str(config.get_trajectory_path(args.object_name, args.num_viewpoints, "gtsp_final.csv"))
    else:
        # Fallback to old structure
        traj_base = os.path.basename(args.trajectory)
        collision_free_csv = os.path.join(traj_dir, traj_base.replace('.csv', '_collision_free.csv'))

    # Auto-generate collision report path
    collision_report_txt = os.path.join(traj_dir, "collision_curobo.txt")

    print(f"\n{'='*60}")
    print("COLLISION CHECKING & REPLANNING (Section 4)")
    print(f"{'='*60}")
    print_key_value("Input trajectory", args.trajectory)
    print_key_value("Robot config", args.robot_config)
    print_key_value("Obstacle mesh", mesh_file)
    print_key_value("Output trajectory", collision_free_csv)
    print_key_value("Collision report", collision_report_txt)
    print(f"{'='*60}\n")
    ...
    print_key_value("Total waypoints", len(trajectory))
    print()
    ...
    # Step 3: Check trajectory with adaptive interpolation and replanning
    print_section_header("CHECKING TRAJECTORY", width=60)
    results = checker.check_trajectory(
        trajectory=trajectory,
        verbose=args.verbose,
        exclude_last_joint=args.exclude_last_joint if args.exclude_last_joint else None,
    )
    ...
    print(f"{'='*60}")
    print_key_value("Total waypoints", results['total_waypoints'])
    print_key_value("Total configurations checked", results['total_configs_checked'])
    print_key_value("Interpolated configs", results.get('total_interpolated_configs', 0))
    print_key_value("Collisions at waypoints", results['num_collisions'])
    print_key_value("Segment collisions", results.get('num_segment_collisions', 0))
    print_key_value("Total collisions", results['total_collisions'])
    print_key_value("Collision rate", f"{results['collision_rate']:.2f}%")

    if results.get('replan_enabled', False):
        print("\nReplanning Statistics:")
        print_key_value("  Iterations performed", results.get('replan_iterations', 0))
        print_key_value("  Segments replanned (success)", results.get('replan_success_count', 0))
        print_key_value("  Segments failed", results.get('replan_fail_count', 0))
        print_key_value("  Success rate", f"{results.get('replan_success_rate', 0)*100:.1f}%")

    print(f"{'='*60}\n")

    # Step 4: Save collision-free trajectory
    if 'final_trajectory' in results:
        final_traj = results['interpolated_trajectory']
        print_section_header("SAVING COLLISION-FREE TRAJECTORY", width=60)
        save_trajectory_csv(
            trajectory=final_traj,
            output_path=collision_free_csv,
            joint_names=joint_names,
        )
        print_key_value("Output path", collision_free_csv)
        print_key_value("Final waypoints", len(final_traj))
        print()

    # Step 5: Save collision report
    print_section_header("SAVING COLLISION REPORT", width=60)
    report_path = save_collision_report(
        trajectory_path=args.trajectory,
        results=results,
        checker_type="curobo",
    )
    print_key_value("Report path", str(report_path))
    print()

    print_success("✓ Section 4 완료!")
