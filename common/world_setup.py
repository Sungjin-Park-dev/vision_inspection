...
def setup_table_only(
    table_position: Optional[np.ndarray] = None,
    table_dimensions: Optional[np.ndarray] = None,
    verbose: bool = True
) -> WorldConfig:
    """
    Setup collision world with table only (no other obstacles)
    ...
    Returns:
        WorldConfig containing only table

    Example:
        >>> world_cfg = setup_table_only()
    """
    # Apply defaults
    if table_position is None:
        table_position = config.TABLE_POSITION.copy()
    ...
    if verbose:
        print("\nSetting up collision world (table only)...")

    # Load base world config
    world_cfg = WorldConfig.from_dict(
        load_yaml(join_path(get_world_configs_path(), "collision_table.yml"))
    )
    world_cfg.cuboid[0].pose[:3] = table_position
    world_cfg.cuboid[0].dims[:3] = table_dimensions
    world_cfg.cuboid[0].name = "table"
