...
### 파일 정보
- **위치**: `/isaac-sim/curobo/vision_inspection/scripts/viewpoints_to_tsp.py`
- **라인 수**: ~1050 lines
...
#### 1. Random 점으로 빠른 테스트
```bash
/isaac-sim/python.sh scripts/viewpoints_to_tsp.py \
  --random \
  --num_points 50 \
  --algorithm nn \
  --num_starts 10 \
...
```bash
# NN only
/isaac-sim/python.sh scripts/viewpoints_to_tsp.py --random --num_points 100 \
...
```bash
usage: viewpoints_to_tsp.py [-h] [--mesh_file MESH_FILE] [--num_points NUM_POINTS]
